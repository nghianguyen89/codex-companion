...
export function BeyondComparePage() {
  ...
  const [ready, setReady] = useState<BeyondCompareReadiness | null>(null); const [acknowledged, setAcknowledged] = useState(false); const [busy, setBusy] = useState(false); const [preview, setPreview] = useState<BeyondCompareBundlePreview | null>(null); const [inspection, setInspection] = useState<BeyondCompareBundleInspection | null>(null); const [recovery, setRecovery] = useState<BeyondCompareRecoveryPreview | null>(null); const [confirmation, setConfirmation] = useState(""); const [result, setResult] = useState(""); const [error, setError] = useState("");
  ...
  return <>
    ...
    <section className="backup-preview"><h2>{t("beyondCompare.export")}</h2><p>{t("beyondCompare.exportDescription")}</p><label className="component-option"><input type="checkbox" disabled={busy} checked={acknowledged} onChange={(event) => { setAcknowledged(event.target.checked); setPreview(null); }} />{t("beyondCompare.acknowledgement")}</label><button disabled={busy || !acknowledged} onClick={() => void run(async () => setPreview(await previewBeyondCompare(true)))}>{t("beyondCompare.choosePackage")}</button>
